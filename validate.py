...
class FormValidator:
    ...
    def is_bad_username(username):
        ...
        if not re.match(r"^[a-zA-Z0-9_-]*$", username):
            error += "No special characters are allowed except for '_' and '-'."
    ...
    def is_invalid_password(pw):
        if not re.match(r"(?=^.{8,100}$)(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&amp;*()_+}{&quot;:;'?/&gt;.&lt;,])(?!.*\s).*$", pw):
            return "Passwords must be at least 8 characters, and have at least 1 of each: lowercase letter, uppercase letter, digit, and special character."
        else:
            return False
    @staticmethod
